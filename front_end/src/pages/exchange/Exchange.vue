...
export default {
  ...
  methods: {
    ...
    startWebsock() {
      ...
      this.datafeed = new Datafeeds.WebsockFeed(that.host+'/market',this.currentCoin,socket);
      // this.getKline();
	  ...
	  socket.on("disconnect",()=>{
        ...
        that.getKline();
        //订阅价格变化消息
        socket.on("/topic/market/thumb", function (msg) {
    ...
    buyWithLimitPrice() {
	  ...
	  if (
		this.accMul(params["price"],params["amount"]) + this.accMul(this.accMul(params["price"],params["amount"]), this.symbolFee)
        >
        this.wallet.base
      ) {
        //手续费不足
    ...
    buyWithMarketPrice() {
      ...
      var that = this;
      if (
        this.accAdd(params["amount"],this.accMul(params["amount"], this.symbolFee)) >
        this.wallet.base
      ) {
        //手续费不足
        this.$Modal.confirm({
