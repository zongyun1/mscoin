<template>
  <div class="envelope">
    ...
    <p style="margin-top: -80px;margin-bottom: 80px;text-align:center;color: rgb(255, 136, 79);font-size:13px;">该红包通过MSZLU.COM发出</p>
    ...
    <router-link style="font-size:14px;text-align:center;color: #EEE;margin-top:15px;text-decoration:underline;" to="/app">© MSZLU.COM | 下载APP</router-link>
    </p>
...
export default {
  data() {
    return {
      spinShow: false,
      hasReceived: false,
      inviteLink: "https://www.MSZLU.COM/envelope/",
      envelopeNo: "",
      country: "",
    ...
    getEnvelope(){
        ...
        this.$http.post(this.host + "/uc/redenvelope/query", param).then(res => {
          ...
          if (res.status == 200 && res.body.code == 0) {
            ...
            window.document.title = "【" + this.envelopeInfo.totalAmount + " " + this.envelopeInfo.unit + "】" + this.envelopeInfo.name + "  — MSZLU.COM交易所"
            this.getEnvelopeDetailList();
          } else {
              this.$Message.error(res.body.message);
          }
