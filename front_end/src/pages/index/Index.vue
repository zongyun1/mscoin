...
export default {
  ...
  methods: {
    ...
    startWebsock() {
	  ...
	  socket.on("/topic/market/thumb", (...args) => {
		  console.log(args[0])
    ...
    collect(index, row) {
      ...
      this.$http
        .post(this.host + this.api.exchange.favorAdd, JSON.stringify(params))
        .then(response => {
          var resp = response.body;
          if (resp.code == 0) {
    ...
    cancelCollect(index, row) {
      ...
      params["symbol"] = row.symbol;
      this.$http
        .post(this.host + this.api.exchange.favorDelete, JSON.stringify(params))
        .then(response => {
          var resp = response.body;
          if (resp.code == 0) {
